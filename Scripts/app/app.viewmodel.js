function AppViewModel(dataModel) {
    ...
    self.navigateToLoggedIn = function (userName, role, accessToken, persistent) {
        self.errors.removeAll();

        if (accessToken) {
            dataModel.setAccessToken(accessToken, persistent);
            //Saving users role for future useing
        ...
        switch (sessionStorage["userRole"]) {
           case "Administrator":
                self.navigateToAdministration();
                break;
            case "Merchandiser":
                self.navigateToBuying();
                break;
            case "Customer":
                self.navigateToNumber();
                break;
            case "Supervisor":
                self.navigateToNumber();
                break;
            default:
              self.navigateToExample();
            //    break;
        }
    };
